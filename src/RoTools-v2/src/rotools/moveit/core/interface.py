...
class MoveGroupInterface(object):
    ...
    def __init__(
        ...
        self.left_arm_running_state = True
        self.right_arm_running_state = True
        ...
        if not ee_links:
            ...
            if self.group_num == 3:
                for i, group in enumerate(self.move_groups[:-1]):
    ...
    def group_go_to_all_joint_poses(self, group_name, goals, tolerance=0.01):
        ...
        if isinstance(goals, GeometryMsg.PoseArray):
            goals = goals.poses
        left_ee_link = 'hand_L_ee_link'
        right_ee_link = 'hand_R_ee_link'
        left_group = 'left_arm'
        right_group = 'right_arm'
        group = self._get_group_by_name(group_name)
        group.clear_pose_targets()
        if goals[0] is not None and goals[1] is not None:
            group.set_pose_target(goals[0], end_effector_link=left_ee_link)
            group.set_pose_target(goals[1], end_effector_link=right_ee_link)
        elif goals[0] is not None and goals[1] is None:
            group.set_pose_target(goals[0], end_effector_link=left_ee_link)
        elif goals[0] is None and goals[1] is not None:
            group.set_pose_target(goals[1], end_effector_link=right_ee_link)
        else:
            rospy.logerr('no known type goals')
        # group.go(wait=False)
        group.go(wait=True)
        group.stop()
        group.clear_pose_targets()
        ...
        left_ok = self._wait_pose_goal_execution(left_group, goals[0], tolerance)
        right_ok = self._wait_pose_goal_execution(right_group, goals[1], tolerance)
        return left_ok and right_ok

        # both_ok = self._wait_real_robot_pose_goal_execution(group_name)
        # return both_ok

    def _wait_real_robot_pose_goal_execution(self, group_name):
        # for real robot ##
        if group_name == 'dual_arm':
            rospy.loginfo("dual arm enter blocking, waiting...")
            while self.left_arm_running_state or self.right_arm_running_state:
                rospy.sleep(0.025)
            self.left_arm_running_state = True
            self.right_arm_running_state = True
            rospy.loginfo("dual arm break blocking, execute success")
            return True
        if group_name == 'arm':
            rospy.loginfo("arm enter blocking, waiting...")
            while self.running_state:
                rospy.sleep(0.025)
            self.running_state = True
            rospy.loginfo("arm break blocking, execute success")
            return True
        if group_name == 'left_arm':
            rospy.loginfo("left arm enter blocking, waiting...")
            while self.left_arm_running_state:
                rospy.sleep(0.025)
            self.left_arm_running_state = True
            rospy.loginfo("left arm break blocking, execute success")
            return True
        if group_name == 'right_arm':
            rospy.loginfo("right arm enter blocking, waiting...")
            while self.right_arm_running_state:
                rospy.sleep(0.025)
            self.right_arm_running_state = True
            rospy.loginfo("right arm break blocking, execute success")
            return True
    ...
    def group_go_to_joint_states(self, group_name, goal, tolerance=0.01):
        ...
        # real robot move
        group.go(list(goal), wait=False)
        group.stop()
        return self._wait_real_js_goal_execution(group_name, goal, tolerance)

    def sim_group_go_to_joint_states(self, group_name, goal, tolerance=0.01):
        group = self._get_group_by_name(group_name)
        group.clear_pose_targets()
        group.go(list(goal), wait=True)
        group.stop()
        return self._sim_wait_real_js_goal_execution(group_name, goal, tolerance)

    def _sim_wait_real_js_goal_execution(self, group_name, js_goal, tol):
        # ## For simulation ##
    ...
    def _wait_real_js_goal_execution(self, group_name, js_goal, tol):

        # for real robot ##
        if group_name == 'arm':
            rospy.loginfo("enter blocking, waiting...")
            while self.running_state:
                rospy.sleep(0.025)
            self.running_state = True
            rospy.loginfo("break blocking, execute success")
            return True
        if group_name == 'left_arm':
            rospy.loginfo("left_arm enter blocking, waiting...")
            while self.left_arm_running_state:
                rospy.sleep(0.025)
            self.left_arm_running_state = True
            rospy.loginfo("left_arm break blocking, execute success")
            return True
        if group_name == 'right_arm':
            rospy.loginfo("right_arm enter blocking, waiting...")
            while self.right_arm_running_state:
                rospy.sleep(0.025)
            self.right_arm_running_state = True
            rospy.loginfo("right_arm break blocking, execute success")
            return True
        if group_name == 'dual_arm':
            rospy.loginfo("dual_arm enter blocking, waiting...")
            while self.left_arm_running_state or self.right_arm_running_state:
                rospy.sleep(0.025)
            self.left_arm_running_state = True
            self.right_arm_running_state = True
            rospy.loginfo("dual_arm break blocking, execute success")
            return True
    ...
    def _execute_group_pose(self, group_name, goal, tolerance=0.01, constraint=''):
        ...
        return self._sim_wait_pose_goal_execution(group_name, goal, tolerance)
    ...
    def _sim_wait_pose_goal_execution(self, group_name, pose_goal, tolerance):
        # # # for simulation #
        group = self._get_group_by_name(group_name)
    ...
    def build_absolute_path_for_group(self, group_name, poses, stamp=None, avoid_collisions=True, eef_step=0.1):
        ...
        # print(type(plan), type(plan.joint_trajectory), type(plan.joint_trajectory.points))
        # tra_points = plan.joint_trajectory.points
        # print(tra_points, type(tra_points[0]), tra_points[0].positions)
        # joint_angle = []
        # joint_angle_deg = []
        # for i, point in enumerate(plan.joint_trajectory.points):
        #     joint_angle.append(plan.joint_trajectory.points[i].positions)
        #     print('point are', i, joint_angle)
        #     for angle in joint_angle[i]:
        #         joint_angle_deg.append(angle*180/np.pi)
        #     print('point are', i, joint_angle_deg)
        #
        # assert 1<0
