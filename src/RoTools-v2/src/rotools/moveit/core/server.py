...
class MoveItServer(object):
    ...
    def __init__(self, kwargs):
        ...
        #
        # self._srv_get_left_arm_running_state = rospy.Service('get_left_arm_running_state', SetBool, self.get_left_arm_running_state_handle)
        # self._srv_get_right_arm_running_state = rospy.Service('get_right_arm_running_state', SetBool, self.get_right_arm_running_state_handle)

        # self._srv_get_left_arm_running_state = rospy.Service('get_running_state', SetBool, self.get_left_arm_running_state_handle)
        # self._srv_get_right_arm_running_state = rospy.Service('get_running_state', SetBool, self.get_right_arm_running_state_handle)
        ...
        # For multiple groups
        self._srv_all_joint_pose = rospy.Service('execute_all_joint_poses', ExecuteAllJointPoses, self.all_joint_poses_handle)
        # self._srv_all_pose = rospy.Service('execute_all_poses_py', ExecuteAllPoses, self.all_poses_handle)

        # For multiple groups
        self._srv_all_pose = rospy.Service('execute_all_poses_py', ExecuteAllPoses, self.all_poses_handle)

        self._srv_group_many_poses = rospy.Service('execute_group_many_poses', ExecuteGroupManyPoses,
                                                   self.group_many_poses_handle)

        self._srv_build_cart_path = rospy.Service('build_cart_path', BuildCartPath,
                                                   self.build_cart_path_handle)
    ...
    def get_left_arm_running_state_handle(self, req):
        if req.data:
            self.interface.left_arm_running_state = True
            rospy.logerr('server handle: left arm changing running state to true')
        else:
            self.interface.left_arm_running_state = False
            rospy.logerr('left arm changing running state to false')
        resp = SetBoolResponse()
        resp.success = True
        resp.message = 'Answer the request successfully'
        return resp

    def get_right_arm_running_state_handle(self, req):
        if req.data:
            self.interface.right_arm_running_state = True
            rospy.logerr('server handle: right arm changing running state to true')
        else:
            self.interface.right_arm_running_state = False
            rospy.logerr('right arm changing running state to false')
        resp = SetBoolResponse()
        resp.success = True
        resp.message = 'Answer the request successfully'
        return resp
############################################
    ...
    def group_js_handle(self, req):
        ...
        ok = self.interface.sim_group_go_to_joint_states(req.group_name, req.goal, req.tolerance)

        resp = ExecuteGroupJointStatesResponse()
        resp.result_status = resp.SUCCEEDED if ok else resp.FAILED
        return resp

    def build_cart_path_handle(self, req):
        if not req.tolerance:
            req.tolerance = 0.01
        if not req.eef_step:
            req.eef_step = 0.1
    ...
    def group_many_poses_handle(self, req):
        ...
        if req.goal_type == req.GLOBAL_BASE:
            ...
            ok = self.interface._sim_wait_pose_goal_execution(req.group_name, req.goals.poses[waypoints_number - 1], req.tolerance)
        else:
            rospy.logerr('Unknown goal type for group pose: {}'.format(req.goal_type))
            ok = False
    ...
    def all_joint_poses_handle(self, req):
        if not req.tolerance:
            req.tolerance = 0.01
        ok = self.interface.group_go_to_all_joint_poses(req.group_name, req.goals, req.tolerance)
        resp = ExecuteAllJointPosesResponse()
        resp.result_status = resp.SUCCEEDED if ok else resp.FAILED
        return resp
    ...
    def group_plan_handle(self, req):
        if req.is_absolute:
            plan = self.interface._build_cartesian_path_for_group(req.group_name, req.poses, req.stamp,
